// puzzle at http://adventofcode.com/day/6
...
func ProcessInstruction(instruction string) {
	...
	if len(tokens) < 4 {
		fmt.Printf("Warning: Instruction too short ('%v')\n", instruction)
		return
	}
	...
	switch tokens[0] {
	case "toggle":
		r.p1 = PointFromString(tokens[1])
		r.p2 = PointFromString(tokens[3])

		Toggle(r)

	case "turn":
		r.p1 = PointFromString(tokens[2])
		r.p2 = PointFromString(tokens[4])
		...
		case "on":
			TurnOn(r)
		default:
			fmt.Printf("Warning: Unknown turn instruction ('%v')\n", instruction)
			return
		}

	default:
		fmt.Printf("Warning: Unknown instruction ('%v')\n", instruction)
		return
	}
}
...
/*
	...
	1: true
*/
func SetGrid(r Rectangle, value int) {
	lowX := int(math.Min(float64(r.p1.x), float64(r.p2.x)))
	lowY := int(math.Min(float64(r.p1.y), float64(r.p2.y)))
	highX := int(math.Max(float64(r.p1.x), float64(r.p2.x)))
	highY := int(math.Max(float64(r.p1.y), float64(r.p2.y)))

	for row := lowX; row <= highX; row++ {
...
func Toggle(r Rectangle) {
	SetGrid(r, -1)
}

func TurnOn(r Rectangle) {
	SetGrid(r, 1)
}

func TurnOff(r Rectangle) {
	SetGrid(r, 0)
}
...
func PointFromString(s string) Point {
	...
	if err != nil {
		fmt.Println("Warning: can't convert %v to integer, from %v", point[0], s)
		x = 0
	}
